...
def process(
    ...
    BOX_TRESHOLD = 0.03
    dino_labled_img, label_coordinates, parsed_content_list = get_som_labeled_img(image_save_path, yolo_model, BOX_TRESHOLD = BOX_TRESHOLD, output_coord_in_ratio=False, ocr_bbox=ocr_bbox,draw_bbox_config=draw_bbox_config, caption_model_processor=caption_model_processor, ocr_text=text,use_local_semantics=True, iou_threshold=0.1)
    image = Image.open(io.BytesIO(base64.b64decode(dino_labled_img)))
    #image.save("OmniParser/OmniParser/test_images/screenshot_annotated.png", format="PNG")
...
with gr.Blocks() as demo:
    ...
    submit_button_component.click(
        ...
        outputs=[label_coordinates_output_component]
    )
